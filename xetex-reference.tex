...
\ifx\VERSION\undefined \def\VERSION{testbuild}\fi
\date{\VERSION}
...
The descriptions here are intended to be a reasonably complete list of
the new primitives and features of \xetex. Descriptions are still rather
aenemic, however.
...
(‘alphabet’) and by
\hlink{http:s//www.microsoft.com/typography/otspec/languagetags.htm}{language}.
...
\section{Cross-compatibility with \pdftex, \luatex, etc.}

All the major (non-frozen) \TeX\ engines support most of the
functionality described in this section (in one way or another), so that
macro writers (e.g., \LaTeX) can make use of it.
